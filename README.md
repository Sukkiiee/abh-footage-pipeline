...
**YouTube on a hosted deployment (Render, Vercel, etc.) will likely fail with "Sign in to confirm you're not a bot"** even for public videos -- YouTube challenges requests from datacenter/cloud IPs far more aggressively than from a home connection, and this is unrelated to anything in this app's code. Fix: set `YT_DLP_COOKIES_CONTENT` to your own exported YouTube session cookies (log into youtube.com, export with a browser extension like "Get cookies.txt LOCALLY", paste the file contents into the env var -- see `.env.example`). Treat it like a credential; it's your personal session, and cookies do eventually expire and need re-exporting.
...
**This repo must be deployed as a Docker-based Render service, not Render's default "native environment."** The bundled static ffmpeg/ffprobe binaries (fine on most hosts) reliably crash with `SIGSEGV` in Render's native runtime container -- confirmed in production against two different static builds. A real system ffmpeg fixes it, but Render's native *build* sandbox has a read-only `/var/lib/apt`, so `apt-get install` can't run there either. The included `Dockerfile` installs ffmpeg via apt at *image build time* instead (full root access there), baking a working, dynamically-linked ffmpeg into the image -- `lib/media.ts` automatically prefers it (`/usr/bin/ffmpeg`/`/usr/bin/ffprobe`) over the bundled static binaries when present.

1. Push this repo to GitHub (the `Dockerfile` and `.dockerignore` at the repo root are what Render will pick up).
2. [render.com](https://render.com) → **New → Web Service** → connect the repo → Render should auto-detect the `Dockerfile` and offer **Docker** as the environment (pick it explicitly under **Environment** if it doesn't autoselect). Do not use the "Node" native environment for this repo.
3. Leave build/start commands blank -- the Dockerfile's own `RUN npm run build` and `CMD ["npm", "run", "start"]` handle both.
